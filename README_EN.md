...
MIMS is deployed for Claude Code CLI and Codex App users, consisting of three core components: persona configuration injected into the main Agent (`CLAUDE.md` / `AGENTS.md`), structured workflow (Skill), and sub-agents handling atomic tasks.
...
|-------|-----------|-----------|
| Persona & Base Behavior | CLAUDE.md / AGENTS.md | T0 injection into main Agent, always active |
...
│   ├── AGENTS.md                       # MIMS persona (Codex, generated on install)
│   └── .claude/
│       ├── agents/
...
9. **R9 Delivery** - Deliver prototype, collect feedback

**Output**: `prototype/` (interactive prototype with process-driven workbench and review feedback recording)
...
**Read Documentation**:
- Understand project architecture from `CLAUDE.md`
...
**Deployment**:
- **Recommended**: Use installation script (see `impl/README.md`)
- **Manual**: Copy contents of `impl/` directory to project root
- **AI Agent**: AI agents can read `impl/README.md` to auto-install
