...
MIMS v1.4 では Codex 互換性とモデル品質管理を強化しました。

- Codex では `AGENTS.md` により自然言語トリガーを利用できます。
- サブエージェントが使えない場合でも、同等のルールに基づく fallback を実行します。
- フェーズ完了には `metadata.validation` の記録が必要です。
- SRS/SDD にはモデル id を保持し、`domain-model.yaml` へ追跡できます。
- プロトタイプは機種依存の絶対パスではなく、既定で相対ディレクトリ `prototype/` に出力します。
