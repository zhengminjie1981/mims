...
| `prototype/feedback.md` | 评审反馈文件（初始为空模板，用户体验时自动填充） |
...
**流程数据提取**：提取流程中文名、parent_scenario_chinese（从父场景获取）、所有步骤（含 actor、action、business_logic）。步骤中 target_module 等于当前模块名的标记为"当前模块步骤"。
...
│  │ ─── 已记录的反馈列表 ───                       │ │
│  │ • [{页面名}] [{优先级}] {问题描述}  [删除]      │ │
│  │ 提示：反馈已自动保存到 prototype/feedback.md    │ │
│  └────────────────────────────────────────────────┘ │
│                                                      │
└──────────────────────────────────────────────────────┘
```
...
- **工作台样式**：
  ...
  - **评审反馈样式**：
    - `.feedback-section`（评审反馈区域，白色背景卡片，圆角阴影）
    - `.feedback-form`（表单行：下拉框+优先级选择+文本框+按钮，flex 布局）
    - `.feedback-item`（反馈条目：页面名标签 + 优先级标签 + 描述 + 删除按钮）
    - `.feedback-badge`（反馈数量徽标，红色圆形，用于"记录反馈"按钮旁）
    - `.priority-high`（红色）、`.priority-medium`（橙色）、`.priority-low`（蓝色）优先级标签
    - `.feedback-tip`（底部提示文字，灰色小字）
...
- **评审反馈功能**：
  - `addFeedback(page, priority, description)`：添加反馈，存入 localStorage（键名 `mims_feedback`），自动触发下载更新后的 `feedback.md`
  - `removeFeedback(index)`：删除反馈，更新 localStorage 并触发下载
  - `renderFeedbackList()`：渲染已记录的反馈列表
  - `generateFeedbackMd()`：将 localStorage 中的反馈数据生成 markdown 格式文本
  - `downloadFeedback()`：将 feedback.md 内容作为文件下载（覆盖旧文件）
  - 页面选择下拉框列出所有模块页面名（含工作台）
  - 模块页面底部需求抽屉旁增加"记录反馈"快捷按钮，点击弹出反馈输入框并自动填入当前页面名
- **需求抽屉功能**：
