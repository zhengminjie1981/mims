...
| prototype_plan | object | 否 | 原型阶段规划数据（R6 完成后写入），含 permissions_matrix、page_functions、page_flows、page_structures、page_interactions |
  ...
  objects:
    - id: string              # 必需，格式 obj_\d+
      ...
      lifecycle:              # 可选，对象生命周期配置
        ...
        updatable_fields: []      # 可修改的字段列表，空 = 全部
