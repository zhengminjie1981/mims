...
function Get-GitlabLatestTag {
    $j = Invoke-GitlabRequest "$GitlabApi/repository/tags?per_page=50"
    if (-not $j) { return "" }
    try {
        $tags = ($j | ConvertFrom-Json) | ForEach-Object { $_.name -replace '^v', '' }
        $valid = $tags | Where-Object { $_ -match '^\d+\.\d+(\.\d+)?$' }
        if (-not $valid) { return "" }
        return ($valid | Sort-Object { [version]($_ -replace '\.\d+$', '') } -Descending | Select-Object -First 1)
    } catch { return "" }
}
...
# 实际安装的版本：从包内 .mims-version 读取（而非 installer 脚本自身的 $MIMS_VERSION）
$packageVersion = $MIMS_VERSION
$versionFilePkg = Join-Path $packageDir ".mims-version"
if (Test-Path $versionFilePkg) { $packageVersion = (Get-Content $versionFilePkg -Raw).Trim() }
if (-not $packageVersion) { $packageVersion = $MIMS_VERSION }
if ($packageVersion -ne $MIMS_VERSION) { Info-Msg "实际安装版本：$packageVersion（installer 自身版本 $MIMS_VERSION，以包内容为准）" }
...
# 安装 .mims-version 到 ~/.claude/skills/ 和 ~/.agents/skills/（Get-MimsInstalledVersion 从此处读取）
$versionSrc = Join-Path $packageDir ".mims-version"
if (Test-Path $versionSrc) {
    Copy-Item $versionSrc "$CLAUDE_DIR\skills\.mims-version" -Force
    Copy-Item $versionSrc "$AGENTS_DIR\skills\.mims-version" -Force
}
