...
try: print(json.load(sys.stdin).get("tag_name",""))
    ...
    if [ -z "$t" ]; then
        t=$(gitlab_fetch "$GITLAB_API/releases" - 2>/dev/null | python3 -c 'import json,sys
try:
    d=json.load(sys.stdin); print(d[0]["tag_name"] if d else "")
except Exception: print("")' 2>/dev/null || true)
    fi
...
verify_integrity() {  # PACKAGE_DIR
    ...
    if [ ! -f "$sums" ]; then
        warn "发布包未含 SHA256SUMS，跳过完整性校验（旧版包或本地源码）"
        return 0
    fi
...
mkdir -p "$MIMS_HOME"
cat > "$MIMS_HOME/update.sh" << 'UPDATE_EOF'
#!/bin/bash
