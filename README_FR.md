...
MIMS est deploye pour Claude Code CLI et Codex App, compose de trois composants principaux : la configuration de persona injectee dans l'Agent principal (`CLAUDE.md` / `AGENTS.md`), le workflow structure (Skill), et les sous-agents traitant les taches atomiques.
...
|--------|------------|---------------|
| Persona & Comportement de Base | CLAUDE.md / AGENTS.md | Injection T0 dans l'Agent principal, toujours actif |
...
│   ├── AGENTS.md                       # Persona MIMS (Codex, genere a l'installation)
│   └── .claude/
│       ├── agents/
...
9. **R9 Livraison** - Livrer le prototype, collecter les retours

**Sortie** : `prototype/` (prototype interactif avec workbench piloté par les processus et enregistrement des retours d'évaluation)
...
- Comprendre l'architecture du projet depuis `CLAUDE.md`
...
**Deploiement** :
- **Recommande** : Utiliser le script d'installation (voir `impl/README.md`)
- **Manuel** : Copier le contenu du repertoire `impl/` a la racine du projet
- **Agent AI** : Les agents AI peuvent lire `impl/README.md` pour s'installer automatiquement
