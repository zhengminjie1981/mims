...
**原则**：附件不是属性类型；简单附件用 `string` 文件路径/URL，复杂附件建独立对象并建立关系。

```yaml
metadata:
  project_name: "合同附件管理系统"
  version: "1.0"
...
domain:
  ...
  objects:
    - id: "obj_010"
      ...
      attributes:
        ...
        - name: "signed_file_url"
          ...
          type: "string"
          required: false
          description: "简单附件场景：记录签署版合同文件的访问链接"
          data_dictionary:
            format: "文件路径/URL"
    ...
    - id: "obj_011"
      ...
      chinese_name: "合同附件"
      module: "ContractModule"
      description: "复杂附件场景：合同可包含多份补充材料并需独立管理"
      attributes:
        - name: "id"
          chinese_name: "附件编号"
          type: "string"
      ...
      rules:
        - id: "rule_010"
          constraint: "单个附件不超过 20MB，支持 PDF、JPG、PNG"
          description: "限制合同附件格式和大小"
          severity: "error"
...
function:
  actors:
    - id: "actor_010"
      name: "contract_clerk"
      chinese_name: "合同专员"
      description: "维护合同和合同附件"
  scenarios:
    - id: "sc_010"
      name: "manage_contract_attachments"
      chinese_name: "管理合同附件"
      description: "合同专员上传、预览和下载合同附件"
      actors: ["contract_clerk"]
      workflow:
        - step: 1
          actor: "contract_clerk"
          action: "上传合同附件"
      preconditions: ["合同已创建"]
      postconditions: ["合同附件已归档"]
  processes: []
```
...
metadata:
  ...
  design_progress:
    preliminary:
      status: "complete"
      completed_steps: ["P1", "P3", "P4", "P5", "P6"]
      skipped_steps: ["P2"]
      checkpoint_at: "2026-03-23T09:30:00Z"
    detailed:
      status: "complete"
      completed_steps: ["D1", "D2", "D3", "D4", "D5"]
      checkpoint_at: "2026-03-23T09:50:00Z"
    prototype:
      status: "complete"
      completed_steps: ["R1", "R2", "R3", "R4", "R5", "R6", "R7", "R8", "R9"]
...
metadata:
  ...
  design_progress:
    ...
    detailed:
      status: "complete"
      completed_steps: ["D1", "D2", "D3", "D4", "D5"]
      checkpoint_at: "2026-03-23T10:00:00Z"
...
domain:
  modules:
    ...
    - id: "mod_003"
      ...
      is_ai_agent: true
      ai_agent_type: "decision"
      ai_justification: "订单审核需要综合判断库存、金额、客户信用等多维因素，规则难以穷举，适合 AI 决策"
      relationships:
        - from: "Order"
          to: "Product"
          type: "many_to_many"
  ...
  objects:
    ...
    - id: "obj_003"
      ...
      lifecycle:
        ...
        readable: true
        updatable: true
        updatable_fields: ["remark", "address"]
        deletable: false
        archivable: true
        audit_fields: true
...
function:
  ...
  processes:
    - id: "proc_001"
      ...
      description: "对「客户下单」场景的端到端细化，从下单到最终收货的完整业务链"
      ...
      steps:
        - step: 1
          ...
          info_passed:
            - from: "customer"
              to: "OrderModule"
              content: "商品选择、数量、收货地址"
          business_logic: "创建订单并调用支付网关完成付款"
        ...
        - step: 2
          ...
          info_passed:
            ...
            - from: "admin"
              to: "OrderModule"
              content: "快递单号、发货确认"
          business_logic: "库存充足则自动审核通过，否则人工审核"
...
external_interfaces:
  - id: "ext_001"
    ...
    connected_module: "OrderModule"
    trigger: "客户点击支付按钮时"
    description: "调用第三方支付完成订单付款"
    request_data:
      - object: "Order"
        fields: ["order_no", "total_amount"]
    response_data:
      - fields: ["transaction_id", "pay_status", "pay_time"]
    related_operations: ["pay"]
    reliability:
      timeout: "30s"
      retry: 3
  ...
  - id: "ext_002"
    ...
    chinese_name: "短信服务"
    type: "api"
    interface_type: "data_exchange"
    direction: "outbound"
    protocol: "HTTPS REST API"
    connected_module: "OrderModule"
    trigger: "订单状态变更时"
    description: "发送订单状态变更短信通知"
    request_data:
      - object: "Customer"
        fields: ["phone"]
    response_data:
      - fields: ["send_status", "msg_id"]
    related_operations: ["pay", "ship", "cancel"]
...
admin_features:
  ...
  notification:
    ...
    triggers:
      - event: "order_paid"
        recipients: ["admin"]
        message_template: "新订单 {order_no} 已支付，请及时发货"
      - event: "order_shipped"
        recipients: ["customer"]
        message_template: "您的订单 {order_no} 已发货，快递单号：{tracking_no}"
  data_config:
    - name: "order_timeout_hours"
      chinese_name: "订单超时时间（小时）"
      default_value: 24
      description: "超过此时间未支付的订单将自动取消"

non_functional:
  concurrent_users: "50-100"
  security_level: "personal_data"
  availability: "working_hours"
  data_retention: "订单数据保留3年"
  notes: "涉及客户手机号和地址信息，需注意隐私保护"
```
...
metadata:
  ...
  design_progress:
    preliminary:
      status: "complete"
      completed_steps: ["P1", "P2", "P3", "P4", "P5", "P6"]
      checkpoint_at: "2026-03-26T09:30:00Z"
    detailed:
      status: "in_progress"
        ...
        - step: 2
          ...
          info_passed:
            - from: "OrderModule"
              to: "admin"
              content: "待审核订单信息"
          business_logic: "金额>1万需二级审批，否则自动通过"
        - step: 3
          action: "客户确认收货"
          actor: "customer"
          objects: ["Order"]
