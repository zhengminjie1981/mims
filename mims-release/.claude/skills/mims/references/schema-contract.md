...
- `metadata.version` 必须匹配 `^\d+\.\d+(\.\d+)?$`，例如 `"1.0"` 或 `"1.5.0"`，不得写成 `"0.1-draft"`。
...
| SRS Requirement Candidate | `req_` | `req_001` | — |
...
`requirements` 是可选顶层结构；旧模型没有 `requirements` 时，视为 `requirements.srs.source_mode = "model"`，继续按当前方式从已验证的 `domain-model.yaml` 生成 `srs.md`。

若存在 `requirements.srs`，必须遵守：

- `source_mode` 只能是 `model`、`adopted_refined`、`manual`、`external`。
- `adoption_status` 只能是 `none`、`candidate_detected`、`adopted`、`refined`、`declined`。
- `candidates[].id` 必须使用 `req_` 前缀，且在 `requirements.srs.candidates[]` 内唯一。
- `candidates[].status` 只能是 `candidate`、`adopted`、`refined`、`rejected`、`needs_clarification`、`superseded`。
- `candidates[].category` 只能是 `functional`、`non_functional`、`interface`、`data`、`business_rule`、`constraint`、`assumption`、`other`。
- `candidates[].confidence` 如存在，必须在 0–1 范围内。
- `rejected`、`superseded`、`needs_clarification` 候选不得作为已确认需求写入正式 `srs.md` 正文；最多列入待确认或变更记录。
- `source_mode = adopted_refined` 时，P2 必须先向用户说明“采纳原始文档为 SRS 初稿来源”的含义并取得确认；不得静默覆盖原始文档。
- 采纳模式不得绕过文档生成 Gate：`srs.md` 仍只能在 `metadata.validation.preliminary.error_count = 0` 后生成或精炼，且必须基于已验证的 `domain-model.yaml`。
...
## 12. Description Length Guidelines
