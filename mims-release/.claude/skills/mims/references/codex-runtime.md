...
如果项目入口 managed block 为 `state=paused`，普通产品描述、业务讨论或开发问题不得自动进入 MIMS；只有上述明确命令或明确“按 MIMS/迷悟师继续”时才恢复。
...
4. 检测项目激活状态：解析 `CLAUDE.md` / `AGENTS.md` 的 `<!-- MIMS-START ... -->` managed block；无 state 的旧 block 视为 `active`，`state=paused` 时仅响应明确 lifecycle 或 MIMS 命令
