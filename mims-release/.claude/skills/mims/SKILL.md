...
**触发**：解析 `$ARGUMENTS` 确定意图后、执行任何分支（展示介绍 / 启动设计 / model / status / ...）之前，先执行一次安装完整性自检。这是加载第一道关卡，确保 MIMS 文件齐全、版本一致、无残留。

**执行**：
1. 优先调用脚本：`python ~/.mims/mims-lifecycle.py selfcheck`（Windows 用 `python` 或 `py`）。脚本只读、不联网、不写文件，输出分级文本与 `SUMMARY: errors=N warns=N infos=N`；退出码 `errors>0 → 1`，否则 `0`。
2. **兜底**（脚本不存在 = 旧版/未装好）：内联最小检查——验证当前端 `SKILL.md` 可读、`references/schema-contract.md` 与 `references/schema.md` 存在、至少一个 `agents/mims-*.md` 存在；任一缺失即视为 ERROR。

**按结果分流**：
- `SUMMARY` 中 `errors=0` 且 `warns=0` → **静默通过**，继续执行原分支（不打扰用户）。
- `errors>0` 或 `warns>0` → 进入下方修复流程；`[INFO]` 项（单端安装、edge/dev）只一句提示，不阻塞、不强制修复。

> 自检覆盖**全局安装健康度**（两端文件齐全、frontmatter、版本一致、references/agents 对称、重复 Skill 残留、`~/.mims` 工具链）；**项目激活状态**（managed block / 入口文件）仍由下方「项目初始化」负责，两者职责不同。

### 修复流程：本地优先 → 联网兜底

按 selfcheck 输出的修复标签分流，**任何删除或联网操作都必须先中文提示并征得用户确认**，绝不静默执行：

**阶段 A · 本地修复（`[local_fixable]`，零/低风险）**
- **重复 Skill 目录**（如 `mims2` / `mims.bak`）：列出完整路径 → 用户确认 → 删除（Windows `Remove-Item -Recurse -Force`、Linux/macOS `rm -rf`）→ 重跑 selfcheck 验证该项转 OK。
- **两端互恢复**：一端缺核心文件、另一端齐全时，从齐全文一份复制过去 → 重跑 selfcheck。
- 每步本地修复后立即重跑 selfcheck；若仍未转 OK，降级到阶段 B。

**阶段 B · 联网修复（`[needs_reinstall]`，本地无法解决）**
- 触发：核心文件（SKILL.md / schema / agent）缺失或损坏、半套安装、两端版本漂移、`~/.mims` 工具链缺失、两端不对称且无法本地对齐。
- 用户确认后，优先调本地更新器（不裸跑安装器）：Windows `& "$HOME\.mims\update.ps1"`、Linux/macOS/WSL `bash ~/.mims/update.sh`。
- 本地更新器不存在（旧版/工具链缺失）→ 回退展示加固安装命令（GitHub 公网 / GitLab 内网二选一；内网提示需 `MIMS_TOKEN`）。
- 更新/重装完成 → 重跑 selfcheck 验证；仍失败 → 请用户带 selfcheck 输出反馈。

### 用户提示原则

- 对齐人设：非技术、不命令、先复述再行动；路径保留原文，不堆 FBS 术语。
- 修复前复述"我发现…"、列出建议、征得确认；修复后给"现在…"的结果对比。
- `[INFO]` 级（单端安装、edge）只一句提示，不阻塞、不强制修复。
...
```markdown
<!-- MIMS-START state=active version=1.6.0 -->
...
**运行时自检**：
- 核心规则文件（`schema-contract.md` / `schema.md`）与 agent 文件的存在性已由上方「安装完整性自检」在加载时覆盖；此处仅处理运行时能力降级。
- 若当前环境无法委托 `mims-*` 子代理，但可读取 agent 规则文件，则进入 `fallback-manual` 模式。
- 若运行中仍发现 `schema-contract.md` 或 `schema.md` 不可读，不得生成或修改 `domain-model.yaml`，按「安装完整性自检」的联网修复流程处理。
...
2. 检查 `requirements.srs.source_mode = "adopted_refined"` 且 `requirements.srs.adoption_status` 不是 `refined`，或存在 `status: needs_clarification` 的 SRS 候选 → 提示用户："检测到已采纳的 SRS 初稿尚未精炼定稿，建议先处理待确认项并生成/更新 srs.md。是否继续？"
3. 检查 `metadata.design_progress.detailed.status` 为 `complete` 或 `complete_with_warnings`，且 `metadata.documents.sdd.generated` 不为 `true`（或文件不存在）→ 提示用户："详细设计已完成，但设计规格说明书（sdd.md）缺失，建议现在补生成。是否生成？"
4. 用户确认后 → 委托 mims-spec-generator 补生成对应文档 → 更新 `metadata.documents` 字段 → 继续原流程
...
5. 根据 `requirements.srs.source_mode` 执行 SRS 生成/精炼：缺失或 `model` 时按模型生成；`adopted_refined` 时先对齐已采纳候选、P3-P5 确认内容和 `metadata.decisions[]`，排除 rejected/superseded/needs_clarification 后委托 mims-spec-generator 生成或更新 `srs.md`；`manual`/`external` 时不得静默改写，只给出一致性建议或在用户明确要求后更新 `{location}/srs.md`
6. 委托 mims-spec-generator 生成或精炼 `srs.md`（type: srs，传入最小元数据，Spec Generator 自加载模板和模型文件）
7. 展示检查点摘要 → 用户确认
8. 按 validation_result 推导更新 `metadata.design_progress.preliminary.status`，不得手工乐观写入 `complete`
...
每步完成后的质量快照格式、检查内容表和四级置信度决策，见 `references/workflow-common.md` §五。
...
| `/mims srs` | 仅在 `metadata.validation.preliminary.error_count = 0` 后委托 mims-spec-generator 生成或更新 `srs.md`（type: srs，传入最小元数据）；要求 `domain-model.yaml` 已存在且已通过初步验证。若 `requirements.srs.source_mode = adopted_refined`，先处理 adopted/refined/needs_clarification 候选；若为 `manual`/`external`，默认只做一致性检查和更新建议，不静默覆盖 |
...
| SRS/SDD | 委托 `mims-spec-generator` | 传入最小元数据，Spec Generator 自行加载模板和模型文件生成文档；SRS 必须识别 `requirements.srs.source_mode` 并执行 model/adopted_refined/manual/external 分支 |
