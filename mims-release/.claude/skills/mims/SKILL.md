...
1. **检测项目激活状态**：检查当前项目目录的 CLAUDE.md 或 AGENTS.md 是否存在 MIMS managed block（`<!-- MIMS-START ... -->` 到 `<!-- MIMS-END -->`）
   ...
   - **state=paused** → 当前项目已暂停 MIMS 常驻加载；除非用户明确输入 `/mims resume`、`/mims persist`、`/mims status`、`/mims change`、`/mims prototype`，否则不要进入设计流程，提示当前默认处于开发状态
...
```markdown
<!-- MIMS-START state=active version=1.4 -->
...
<!-- MIMS-END -->
```

兼容旧版本：`<!-- MIMS-START -->` 没有 state 时视为 `active`。暂停时替换为 `references/claude-md-paused-template.md` 的短模板；重新持久化时替换为 `references/claude-md-template.md`。
...
| `/mims pause` | 将项目入口中的 active block 替换为 paused stub，写入 `.mims/state.yaml activation_state: paused`；随后询问是否把工作产品搬到 `design/`（opt-in，见"搬迁执行"），搬迁则更新 `design_artifacts.location`。当前会话从此不再主动进入 MIMS，真正减少加载量从下次会话开始生效 |
...
| `/mims persist` | 将 paused/absent/detached 项目重新写入 active block；如 block 不存在则按初始化流程追加；写入 `.mims/state.yaml activation_state: active`；`location` 保持不变（不主动搬回根目录，恢复到根目录需手动移回并清空 location） |
| `/mims detach` | 二次确认后只移除 managed block，写入 `.mims/state.yaml activation_state: detached`；不删除工作产品或 MIMS 安装目录 |

当 paused 状态下收到普通开发问题时，按当前项目开发助手规则回答，不主动切入 MIMS；只有明确 lifecycle 命令或明确“按 MIMS/迷悟师继续”时才恢复。
...
| `/mims persist` | 将当前项目重新持久化为 active：用 `references/claude-md-template.md` 替换 paused block，或在无 block 时按初始化流程追加；写入 `.mims/state.yaml activation_state: active`；`design_artifacts.location` 保持不变（不主动搬回根目录） |
...
| `/mims update` | 展示本地 updater 优先的更新方式。优先提示：Windows `& "$HOME\.mims\update.ps1"`，Linux/macOS `bash ~/.mims/update.sh`；如需内网源可追加 `gitlab`。若本地 updater 不存在，回退展示 GitHub/GitLab 直接安装命令 |
