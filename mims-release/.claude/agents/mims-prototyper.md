...
6. 评审工具配置：启用的标签页列表、上下文联动开关
7. 附件交互计划：附件字段/附件对象、上传/预览/下载/删除/替换功能及验收标准

---

## 输出文件

生成以下文件，写入调用方指定的输出目录 `{output_dir}`（由调用方按 `.mims/state.yaml` 的 `design_artifacts.location` 和 `metadata.prototype_plan.output_dir` 解析后传入，默认 `prototype/`）：

| 文件 | 说明 |
|------|------|
| `{output_dir}/index.html` | 主入口，含侧边栏导航，默认展示第一个模块 |
| `{output_dir}/workbench.html` | 流程驱动工作台页面（仅当 `function.processes` 非空时生成） |
| `{output_dir}/{module}.html` | 每个 module 一个页面（按布局类型生成） |
| `{output_dir}/{module}-form.html` | 新建方式为"独立页面"时额外生成 |
| `{output_dir}/styles.css` | 全局样式 |
| `{output_dir}/app.js` | 交互逻辑 + 模拟数据 + 流程模拟 |
...
- 适合图片展示的场景（如存在图片/照片类附件字段或附件对象，卡片顶部显示图片占位区域）
...
| 设计决策 | 生成实现 |
|---------|---------|
| 查看详情 = 模态框 | 模态框逻辑（`<div class="modal-overlay"><div class="modal">...</div></div>`，添加 `.active` 激活，遮罩层关闭） |
...
CSS、HTML、JS 三个文件必须共用以下类名。生成任何文件前，先确认本表。

| 组件 | 容器类 | 子元素类 | 激活/状态类 | JS 选择器 |
|------|--------|---------|------------|----------|
| 侧边栏导航 | `.sidebar-nav` | `.sidebar-nav-item` | `.sidebar-nav-item.active` | `querySelector('.sidebar-nav-item')` |
| 数据表格 | `.data-table` | `.data-table-row` | `.data-table-row.selected` | `querySelector('.data-table')` |
| 状态标签 | `.status-tag` | — | `.status-tag.initial/.intermediate/.final/.danger` | — |
| 按钮组 | — | `.btn-primary`, `.btn-secondary`, `.btn-danger` | — | — |
| 搜索框 | `.search-box` | — | — | — |
| 模态框 | `.modal-overlay` | `.modal` | `.modal-overlay.active` | `classList.add('active')` |
| 侧边抽屉 | `.side-drawer` | `.drawer-overlay` | `.side-drawer.open` | `classList.add('open')` |
| 需求抽屉 | `.requirements-drawer` | `.review-tab` | `.requirements-drawer.collapsed`（折叠）/ 移除 `.collapsed`（展开） | `classList.toggle('collapsed')` |
| 卡片布局 | `.card-grid` | `.card-item` | — | `querySelectorAll('.card-item')` |
| 看板布局 | `.kanban-board` | `.kanban-column`, `.kanban-card` | — | — |
| 日历布局 | `.calendar-grid` | `.calendar-cell`, `.calendar-header` | — | — |
| 分步向导 | `.step-wizard` | `.step-indicator` | `.step-indicator.active` | — |
| 工具栏 | `.toolbar` | `.toolbar-btn` | `.toolbar-btn.enabled` | — |
| 统计卡片 | `.stat-card` | `.stat-card-icon`, `.stat-card-value` | — | — |
| 体验提示 | `.experience-tip` | `.context-info`, `.copy-btn`, `.tip-guidance` | `.experience-tip.expanded` | — |
| 工作台 | `.workbench-header` | `.task-list`, `.task-item`, `.process-card`, `.progress-bar`, `.process-flow`, `.flow-node`, `.flow-connector` | `.flow-node.completed/.current/.pending` | — |
| 附件组件 | `.attachment-upload` | `.attachment-list`, `.attachment-item`, `.attachment-preview`, `.attachment-actions` | `.attachment-item.removing/.uploading` | `querySelector('.attachment-list')` |

**状态类规则**：
- 模态框激活：统一用 `.active`（CSS: `.modal-overlay.active { display: flex }`）
- 侧边抽屉激活：统一用 `.open`（CSS: `.side-drawer.open { transform: translateX(0) }`）
- 需求抽屉折叠：用 `.collapsed`（CSS: `.requirements-drawer.collapsed { ... }`）
- 导航高亮：用 `.active`（CSS: `.sidebar-nav-item.active { ... }`）

### CRUD 实现标准

所有涉及数据增删改的页面必须实现真正的数据操作，禁止用 Toast 代替。

**标准 CRUD 模式**（所有页面共用同一模式）：

| 操作类型 | 实现模式 | 禁止 |
|---------|---------|------|
| Create | 读表单值 → 构造对象 → `mockData[ObjectName].push(obj)` → `reRender()` | `showToast('创建成功')` 后不做任何数据变更 |
| Read | 从 `mockData[ObjectName]` 读取 → 渲染到模态框/抽屉/详情区 | 无数据展示的空壳弹窗 |
| Update | `mockData[ObjectName].find(…)` → 用表单值覆盖字段 → `reRender()` | `showToast('保存成功')` 后数据未变 |
| Delete | `mockData[ObjectName].splice(index, 1)` → `reRender()` | `showToast('已删除')` 后数据仍在列表中 |
| 状态变更 | 找到对象 → 修改 `status` 字段 → `reRender()` → 按钮按新状态刷新 | 仅 Toast 不改数据 |
| 附件上传 | 构造附件记录（file_name/file_url/file_type/uploaded_at 等）→ `mockData[AttachmentName].push(obj)` 或更新主对象附件数组 → `reRenderAttachments()` | 仅显示"上传成功"但附件列表不增加 |
| 附件删除 | 找到附件记录 → `splice(index, 1)` → `reRenderAttachments()` | 仅 Toast 不从附件列表移除 |
| 附件替换 | 找到附件记录 → 更新 file_name/file_url/file_type/uploaded_at → `reRenderAttachments()` | 仅 Toast 不更新附件数据 |
| 附件预览/下载 | 从 mockData 读取附件记录 → 打开预览弹窗或显示下载反馈 | 无文件名/文件地址上下文的空壳按钮 |

**reRender() 实现规范**：
- 表格布局：清空 `<tbody>` → 重新遍历 `mockData` 生成行
- 卡片布局：清空卡片容器 → 重新遍历 `mockData` 生成卡片
- 看板布局：清空所有列 → 按 `status` 重新分配卡片到各列

**表单值读取规范**：
- 每个 `<input>`/`<select>`/`<textarea>` 必须有 `id` 属性
- 用 `document.getElementById('field-id').value` 读取值
- 新建时构造完整对象（含自动生成的 id）
- 编辑时预填表单值：`document.getElementById('field-id').value = obj.fieldName`

**CRUD 函数命名**：
- `{objectName}Create()` → 创建
- `{objectName}Update(id)` → 编辑
- `{objectName}Delete(id)` → 删除
- `{objectName}ChangeStatus(id, newStatus)` → 状态变更
...
- **需求抽屉**：
  - 折叠态 Tab：半透明黄色底（`rgba(250,204,21,0.85)`），向上突出的 Tab 页签形，hover 0.95
  - 展开态：半透明深色底 + `backdrop-filter: blur(4px)` 毛玻璃效果（`rgba(15,23,42,0.92)`，文字 #e2e8f0，边框 #334155）
  - 展开收起动画（transform + transition），Tab 变为抽屉标题栏保持连续性
  - 标签页切换、需求项卡片样式
- **工作台样式**：
  ...
  - **附件组件样式**：
    - `.attachment-upload`（虚线边框上传区，浅色背景，hover 高亮）
    - `.attachment-list`（附件列表容器，纵向间距）
    - `.attachment-item`（文件图标 + 文件名/类型/大小 + 操作按钮行）
    - `.attachment-preview`（图片缩略图或预览弹窗内容）
    - `.attachment-actions`（预览/下载/删除/替换按钮组）
...
- 折叠态渲染为底部居中半透明黄色 Tab 页签：`<div class="review-tab">📋 评审 · {当前模块名}</div>`
  - Tab 固定在页面底部居中（`position: fixed; bottom: 0; left: 50%; transform: translateX(-50%)`）
  - 半透明黄色底（`background: rgba(250,204,21,0.85)`），文字深色，向上突出约 28-32px
  - hover 时透明度提高到 0.95，暗示可交互
  - 点击 Tab 展开抽屉，Tab 变为抽屉标题栏（视觉连续）
  - Tab 文字跟随当前模块动态更新
- 抽屉头部：由折叠态 Tab 过渡为标题栏，显示"📋 评审 · {当前模块名}"，包含收起按钮
- 标签页导航：6个标签（场景/流程/对象/操作/规则/权限矩阵）
...
**权限矩阵标签页**（新增）：
- 展示角色 × 模块访问权限矩阵（行=角色，列=模块）
- 权限来源：`function.actors[].permissions`，降级策略同 R2（从 scenarios 和 processes 推断）
- 高亮当前角色行和当前模块列
- 可访问标记为"✓"，不可访问标记为"—"
- 仅当角色数 > 1 时显示此标签页
...
- **体验提示功能**（工作台和每个模块页面均包含）：
  ...
  - 组件类型识别规则：通过事件冒泡向上查找最近的语义容器（`.data-table`→"表格"、`.card-item`→"卡片"、`.modal-overlay`→"弹窗"、`.side-drawer`→"侧边抽屉"、`.kanban-column`→"看板列"），取其标题或 aria-label 作为组件名
- **需求抽屉功能**：
  - `toggleDrawer()`：展开/收起抽屉
  - `switchTab(tabName)`：切换标签页（场景/流程/对象/操作/规则/权限矩阵）
  ...
  - `renderPermissions()`：渲染权限矩阵标签页（角色×模块矩阵，高亮当前角色和模块）
  - `updateReviewTab(moduleName)`：更新 Tab 文字为当前模块名（折叠态和展开态标题栏同步更新）
  - 页面切换时自动调用 `updateReviewTab()` 和 `renderRequirements()`
- **附件交互功能**：
  - `renderAttachments(ownerId)` 或 `{objectName}RenderAttachments(ownerId)`：渲染附件列表
  - `{objectName}UploadAttachment(ownerId)`：模拟上传并新增附件记录
  - `{objectName}PreviewAttachment(attachmentId)`：打开预览弹窗或详情区
  - `{objectName}DownloadAttachment(attachmentId)`：显示带文件名的下载反馈
  - `{objectName}DeleteAttachment(attachmentId)`：删除附件记录并重新渲染
  - `{objectName}ReplaceAttachment(attachmentId)`：更新附件记录并重新渲染

---

## 生成顺序

**前置约束**：生成每个文件时，所有类名必须参照「类名契约」节，不得使用契约之外的类名。
...
生成所有文件后，逐项检查以下清单。每项必须通过，不得跳过。

**功能自检**（逐页面检查）：
1. 每个「创建」按钮的 onclick 函数是否实际 push 到 mockData 数组并 reRender？
2. 每个「编辑」按钮的 onclick 函数是否实际修改 mockData 中的对象并 reRender？
3. 每个「删除」按钮的 onclick 函数是否实际从 mockData 中移除元素并 reRender？
4. 每个状态变更按钮是否实际修改对象的 status 字段？
5. 附件上传/删除/替换是否实际修改 mockData 并重新渲染附件列表？
6. 附件预览/下载按钮是否读取具体附件记录并显示文件名或预览内容？
7. 所有 onclick 调用的函数是否在 app.js 中有定义？

**UI 自检**：
8. 每个模态框的打开函数添加的类名与 CSS 激活类名是否一致？（参照类名契约）
9. 每个模态框既能打开也能关闭？
10. 侧边栏导航项的当前页高亮类名与 CSS 一致？
11. 附件组件是否使用 `.attachment-upload`、`.attachment-list`、`.attachment-item`、`.attachment-actions` 等契约类名？
12. 所有 const/let 引用在声明之后使用？（无 TDZ 错误）
...
| # | 检查项 | 结果 |
|---|--------|------|
| 1 | {页面} 创建功能 → push + reRender | ✅ 通过 |
| 2 | {页面} 编辑功能 → find + 修改 + reRender | ✅ 通过 |
| ... | | |
```
