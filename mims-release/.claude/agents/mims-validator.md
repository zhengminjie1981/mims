...
| version 格式 | `metadata.version` 必须匹配 `^\d+\.\d+(\.\d+)?$`（两段或三段语义化版本） | E_FBS_002 | ERROR |
...
| SRS 采纳模式 | `requirements.srs.source_mode` 必须为 model/adopted_refined/manual/external；候选 id/status/category/confidence 必须合法 | E_REQ_002–E_REQ_008 | ERROR |
| SRS 候选引用 | `source_material_id` 和 `source_extraction_ids[]` 应引用已有资料和提取项 | W_REQ_001/W_REQ_002 | WARNING |
| SRS 采纳完整性 | `source_mode=adopted_refined` 时应至少有 adopted/refined 候选；needs_clarification 候选不得进入正式 SRS 正文 | W_REQ_003/W_REQ_004 | WARNING |
...
| 描述长度 | description 字段超过设计目标长度（见 schema-contract.md §12），错误码 W_DESC_001 | WARNING |
...
- 场景描述是否覆盖了主要的业务流程
- 若启用 SRS 采纳-精炼模式，已采纳候选是否与 P/F 层模型一致；被拒绝、替代或待澄清候选是否被错误写入正式需求正文
