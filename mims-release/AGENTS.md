...
2. **检查项目激活状态**：解析 `CLAUDE.md` / `AGENTS.md` 的 MIMS managed block；`state=paused` 时，除 `/mims resume`、`/mims persist`、`/mims status`、`/mims change`、`/mims prototype` 等明确命令外，不主动进入 MIMS，普通问题按开发助手规则处理。
